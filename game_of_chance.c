...
int main(){
	...
	while(choice != 7){
		...
		if((choice < 1)||(choice > 7))
			printf("\n[!!]The number %d is an invaild selection.\n\n",choice);
		else if(choice < 4){
			if(choice != last_game){
				if(choice == 1)
...
//This is the new user regesitration function
...
//It is used primarily for updating the credists after games.
void update_player_data(){
	int fd,i,read_uid;
	char burned_byte;
...
//scanf("%s",&wahtever) will stop input at the first space.
void input_name(){
...
//It expects a message to display,a pointer to the cars array,
...
int take_wager(int available_credits,int previous_wager){
	...
	if(total_wager > available_credits){
		...
		printf("You only have %d available_credits,try again.\n",available_credits);
		return -1;
	}
	return wager;
}

//This function contais a loop to allow the current game to be
...
//It retuns -1 if the player has 0 credits.
int dealer_no_match(){
...
int find_the_ace(){
	...
	int invaild_choice,pick = -1,wager_one = -1,wager_two = -1;
